...
namespace VideogameStudio
{
    public class GameStudio : IDisposable, ICloneable
    {
        ...
        public Todo CurrentTodo { get; private set; }

        public int CurrentWorkForDevelopers { get; private set; }

        public int CurrentWorkForSowtwareTesters { get; private set; }

        public int DevelopmentProductivity { get; private set; }

        public int TestingProductivity { get; private set; }

        public GameStudio(decimal budget, ObservableCollection<Employee> workers, TodoList todos)
        {
            Budget = budget;
            Workers = workers;
            Todos = todos;
        ...
        public void SimulateMonthSkipping()
        {
            ...
            else
            {
                ...
                for (int i = Workers.Count - 1; i >= 0; --i)
                {
                    ...
                    if (employee.Happiness == HappinessLevel.Unhappy)
                    {
        ...
        public void StopCurrentDevelopment()
        {
            if (CurrentWorkForSowtwareTesters == 0)
            {
                throw new NotSupportedException("В данный момент ничего не разрабатывается.");
            }
            Todo savedDevelopment = new Todo(CurrentTodo.Title, CurrentTodo.DevelopmentComplexity, CurrentTodo.Profit);
        ...
        public void WriteDataToFile()
        {
            ...
            foreach (Employee worker in Workers)
            {
                LoggingFileWriter.WriteLine("Имя: {0},  Профессия: {1}, Опыт: {2}, Проработано: {3} мес, Продуктивность: {4}, З/п: {5} руб, Уровень счастья: {6}.\n",
                    worker.Name, worker.Speciality, worker.WorkExperience, worker.WorkedMonths, worker.Productivity, worker.Salary, worker.Happiness);
            }
            if (CurrentTodo != null)
            {
        ...
        public object Clone()
        {
            ...
            foreach (Employee employee in Workers)
            {
                if (employee.Speciality == Specilization.Developer)
                {
                    workers.Add(new Developer(employee.Name, employee.Age, employee.WorkExperience));
                }
                else
                {
                    workers.Add(new SoftwareTester(employee.Name, employee.Age, employee.WorkExperience));
                }
            }
            ...
            foreach (Todo todo in Todos)
            {
                todos.Add(new Todo(todo.Title, todo.DevelopmentComplexity, todo.Profit));
            }

            return new GameStudio(Budget, workers, todos);
        }
    }
}
